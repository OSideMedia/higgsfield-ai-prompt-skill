...
- `python3 validate.py` — health check (frontmatter, paths, JSON schemas)
- `python3 validate.py --strict` — release gate (skips become failures)
- `python3 -m pytest tests/ -q` — Python tooling test suite (CI-run)
- `python3 validate.py --evals` — behavioral eval cases (evals/cases/)
- `python3 build_index.py` — regenerate INDEX.md after any heading change
- `python3 sync_specs.py --type video|image` — regenerate specs/ from the newest dated snapshot
- `python3 refresh_specs.py` — spec-drift tripwire (exit 0 fresh / 3 changed / 1 pull-failed / 4 CLI-shape-changed); `--update-baseline` to accept a reviewed change
...
- Release gate: `python3 validate.py --strict` + `python3 -m pytest tests/ -q` + `python3 validate.py --evals` — all three green before any release
...
- Spec refreshes (Tier 2): dump `models_explore` → dated snapshot in `specs/` → `sync_specs.py` → **audit `evals/cases/` in the same PR** (v3.11.3 lesson) → `refresh_specs.py --update-baseline`
